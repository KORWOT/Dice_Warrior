...
- 자산 oracle: 6개 서로 다른 자식 DiceFaceView 및 각 Graphic의 CanvasRenderer, popup title/detail/result/rollButton, 준비 로비3패널·3탭·표시 필드, layoutVersion1, mapContainer와 동일 GameObject의 CampaignMapView 및4연결 참조를 요구한다.
...
- 재개/유료: ExplorationRoll/Cards·CombatRoll/Cards 네 저장 상태의 진입이 파일 bytes와 전체 상태를 유지하며 Roll만 popup을 연다. 실제 roll 후 oracle와 비교한다. 실제 사건 보상으로 얻은 paid reroll은 비용1회·목표1die만 변경하고 무료 roll popup을 열지 않는다.
