...
- 원본 탐색: 앱/config/visual catalog/UIRoot를 직접 연결로 찾아 선택·Ping하거나 Prefab Stage로 연다. UI 화면 원본은 UIRoot의 UIManager.prefabs에서 concrete type으로 찾는다. Map은 ExplorationUI, ExplorationCards는 FateChoiceUI, Shop은 EncounterUI를 사용한다. 원본 Prefab Stage의 화면을 선택하고 2D SceneView로 프레이밍한다.
