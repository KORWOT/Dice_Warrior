...
namespace FateDice
{
    ...
    public sealed class LocalRunStore
    {
        ...
        HashSet<string> Ids(IEnumerable<string> values,string name)
        {
        ...
        void ValidateState(RunState state)
        {
            ...
            Require(state.schema==RunState.CurrentSchema,"Unsupported run schema "+state.schema+". Expected "+RunState.CurrentSchema+".");
            Require(!string.IsNullOrWhiteSpace(state.runId)&&!string.IsNullOrWhiteSpace(state.profileId),"The run/profile ID is missing.");
            Require(state.config!=null,"The actual configuration snapshot is missing.");
            string[] errors;
            try{errors=state.config.Validate();}
        ...
        static bool EmptyReward(RewardDefinition reward)=>reward==null||(reward.gold==0&&reward.xp==0&&reward.health==0&&
            reward.rerollCharges==0&&string.IsNullOrEmpty(reward.equipmentId)&&string.IsNullOrEmpty(reward.dieId));
        void ValidateGraph(RunState state)
        {
            ...
            {
                Require(EmptyNode(state.selectedNode)&&available.Count==state.config.world.branchCount,"A map requires available branches and no selected path.");
