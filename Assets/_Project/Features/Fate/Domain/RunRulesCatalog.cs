...
namespace FateDice
{
    ...
    {
        /* Stable rule ID; each of the ten kinds exactly once. */ public HandKind kind;
        /* Display text only; not a save ID. */ public string label;
        /* Greater wins. Unique nonnegative priority, separate from fate power. */ public int priority;
        /* Base fate power, clamped to the configured table range after bonus. */ public int fatePower;
    }
    [Serializable] public sealed class DieDefinition
    {
        public string id;
        public string label;
        /* Exactly six face entries. Values 1..6, duplicates allowed. */ public int[] values;
        /* Relative nonnegative face weights; positive finite sum. */ public float[] weights;
    }
    [Serializable] public sealed class GradeRow
    {
        /* Minimum inclusive fate power for this row. Rows must be ascending. */ public int minimumPower;
        /* Common, Uncommon, Rare, Epic, Legendary relative weights. */ public float[] weights;
    }
    [Serializable] public sealed class DiceSettings
    {
        ...
        /* Inclusive power range of both tables. */ public int minimumFatePower, maximumFatePower;
    }
    [Serializable] public sealed class FateSettings
    {
        public GradeRow[] exploration;
        public GradeRow[] combat;
        /* Exactly five grade multipliers. Empty action grades scale numeric effects by offered/base ratio. */ public float[] gradeMultipliers;
        /* Relative nonnegative weights: Combat, Event, Treasure, Shop, Rest. */ public float[] nodeWeights;
        /* Multiply the chosen node weight for each non-guaranteed slot. */ public float selectedTypeBias;
        /* Extra reward multiplier by selected exploration cap. Does not affect combat grades. */ public float[] capRewardMultipliers;
    }
    [Serializable] public sealed class ActionDefinition
    {
        public string id, label;
        public Grade grade;
        /* Damage = Power * coefficient * grade ratio * explicit tag modifiers. Zero means no damage. */ public float damageCoefficient;
        /* Shield = Guard * coefficient * grade ratio * explicit tag modifiers. Zero means no shield. */ public float blockCoefficient;
        /* Nonempty list is authoritative; null/empty uses the legacy coefficients. */ public ActionEffectDefinition[] effects;
        public string[] tags;
    }
    [Serializable] public sealed class EnemyIntent
    {
        ...
        /* Relative selection weight; finite, nonnegative, sum positive. */ public float weight;
        /* Attack uses enemy Power; Defend uses enemy Guard. */ public float coefficient;
    }
    [Serializable] public sealed class EnemyDefinition
    {
        public string id, label;
        /* Authoring minimum: 1. */ public int maxHp;
        /* Authoring minimum: 0. */ public int power, guard;
    ...
    [Serializable] public sealed class CombatSettings
    {
        ...
        /* Choose one owned trial action at start; no draw preference. Rare or lower. */ public string[] trialActionIds;
        public EnemyDefinition[] enemies;
        public string bossEnemyId;
        /* Minimum positive damage/shield after final rounding. No effect if coefficient is zero. */ /* Authoring minimum: 0. */ public int minimumEffect;
    }
    ...
    [Serializable] public sealed class TagModifier
    {
        ...
        /* Additive fractional modifier; 0.2 = +20%. Sum clamped at zero. */ public float bonus;
    }
    [Serializable] public sealed class EquipmentDefinition
    {
        public string id, label;
        public EquipmentSlot slot;
        /* Authoring minimum: 0. */ public int maxHp, power, guard;
        public string[] tags;
        public TagModifier[] modifiers;
    }
    [Serializable] public sealed class LevelStep
    {
        /* XP cost from current to next level. Last row is the final advance, then capped. */ /* Authoring minimum: 1. */ public int xpRequired;
        /* Authoring minimum: 0. */ public int maxHp, power, guard;
    }
    [Serializable] public sealed class RewardDefinition
    {
        /* Authoring minimum: 0. */ public int gold, xp;
        /* HP delta: positive heal, negative hazard. Final HP clamped to max. */ public int health;
        /* Empty means no equipment. */ public string equipmentId;
        /* Earns reroll permission and these charges; zero grants nothing. */ /* Authoring minimum: 0. */ public int rerollCharges;
        /* Empty means no die replacement. The player selects one of six dice. */ public string dieId;
        /* Empty means no action acquisition; duplicates retain single ownership. */ public string addActionId;
    }
    [Serializable] public sealed class EventDefinition
    {
        public string id, label;
        /* Multiline authoring text. */ public string description;
        public NodeType type;
        public Grade grade;
        /* Required for combat events; ignored for other events. */ public string enemyId;
        public RewardDefinition reward;
    }
    [Serializable] public sealed class ShopProduct
    {
        public string id, label;
        /* Authoring minimum: 0. */ public int price;
        public RewardDefinition reward;
    }
    [Serializable] public sealed class GrowthSettings
    {
        public string characterId, characterName;
        /* Authoring minimum: 1. */ public int startingMaxHp;
        /* Authoring minimum: 0. */ public int startingPower, startingGuard, startingGold;
        public LevelStep[] levels;
        public TagDefinition[] tags;
        public EquipmentDefinition[] equipment;
        /* Charges consumed per one-die reroll. Permission must first be earned. */ /* Authoring minimum: 1. */ public int rerollCost;
    }
    [Serializable] public sealed class WorldSettings
    {
        /* Authoring range: 1,100. */ public int eventsToBoss;
        ...
        /* Common..Legendary. Legacy null/empty means base prices at every grade. */ public float[] shopPriceMultipliers;
        public RewardDefinition restTraining;
    }
    // A run-owned rules snapshot. Runtime authoring may derive a definition with presentation fields.
    [Serializable] public class RunRulesCatalog
    {
        // Snapshot schema identity; the complete definitions remain part of each saved run.
        ...
        public RunRulesCatalog DeepCopy()
        {
            var copy = new RunRulesCatalog();
            CopyRulesTo(copy);
            return copy;
        }

        public void CopyRulesTo(RunRulesCatalog target)
        {
            if (target == null) throw new ArgumentNullException(nameof(target));
            target.version = version;
            target.dice = RulesCopy.Dice(dice);
            target.fate = RulesCopy.Fate(fate);
            target.combat = RulesCopy.Combat(combat);
            target.growth = RulesCopy.Growth(growth);
            target.world = RulesCopy.World(world);
        }
        public virtual string[] Validate()
        {
            ...
            Check(growth!=null,"growth: required");Check(world!=null,"world: required");
            if(dice==null||fate==null||combat==null||growth==null||world==null)return errors.ToArray();
            ...
            foreach(var action in combat.actions??Array.Empty<ActionDefinition>())if(action!=null)
            {
                ...
                Check(EffectResolver.IsValid(action),"combat.actions."+action.id+": supported finite nonnegative effects with at least one positive coefficient required");
            ...
            void Reward(RewardDefinition reward,string path)
            {
                ...
                Check(string.IsNullOrEmpty(reward.addActionId)||actionIds.Contains(reward.addActionId),path+": missing action reference");
            }
            Check(world.shopPriceMultipliers==null||world.shopPriceMultipliers.Length==0||(world.shopPriceMultipliers.Length==5&&world.shopPriceMultipliers.All(Positive)),"world.shopPriceMultipliers: five positive finite multipliers required");
